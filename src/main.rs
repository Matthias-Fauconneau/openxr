...
fn main() -> std::result::Result<(), ffmpeg::Error> {
	...
	use ffmpeg::{format, media::Type, codec::context::Context as CodecContext, util::frame::video::Video/*, util::image::Luma*/};
	ffmpeg::init().unwrap();
	let mut stream = format::input(&"../live_high.mp4").unwrap();
	//let context = format::input(&"https://192.168.0.101/api/holographic/stream/live_high.mp4?holo=false&pv=true&mic=false&loopback=false&RenderFromCamera=false").unwrap();
	...
	/*for (stream, packet) in stream.packets() {
		if stream.index() == video_stream_index {
			decoder.send_packet(&packet)?;
			while decoder.receive_frame(&mut frame).is_ok() { dbg!(frame.width(), frame.height(), frame./*plane::<Luma::<u8>>*/data(0).len()); } // YUV420P 1280x720
		}
	}*/
	
	use pollster::FutureExt as _;
	use wgpu as gpu;
	let adapter = gpu::Instance::new(gpu::InstanceDescriptor{backends: gpu::Backends::DX12, dx12_shader_compiler: default()}).request_adapter(&default()).block_on().unwrap();
	let (device, queue) = adapter.request_device(&gpu::DeviceDescriptor{features: gpu::Features::TEXTURE_FORMAT_16BIT_NORM/*|gpu::Features::MULTIVIEW*/, ..default()}, None).block_on().unwrap();
	use wgpu_hal::api::Dx12;
	let vert_shader = device.create_shader_module(gpu::include_wgsl!("fullscreen.wgsl"));
	let frag_shader = device.create_shader_module(gpu::include_wgsl!("sample.wgsl"));
	let ref layout = device.create_bind_group_layout(&gpu::BindGroupLayoutDescriptor{label: None, entries: &[
		gpu::BindGroupLayoutEntry{binding: 0, visibility: gpu::ShaderStages::FRAGMENT, ty: gpu::BindingType::Texture{multisampled: false, view_dimension: gpu::TextureViewDimension::D2, sample_type: wgpu::TextureSampleType::Float{filterable: true}}, count: None},
		gpu::BindGroupLayoutEntry{binding: 1, visibility: gpu::ShaderStages::FRAGMENT, ty: gpu::BindingType::Sampler(wgpu::SamplerBindingType::Filtering), count: None}
	]});
	...
	fn ck(r: Result) { assert_eq!(r, Success); }
	//macro_rules! xr { ($e:expr) => { ck(unsafe{$e}) } }
	ck(unsafe{negotiate_loader_runtime_interface(&NegotiateLoaderInfo::default() as *const _, runtime_request as *mut _)});
	...
	let extensions = array(|capacity, len, buffer| enumerate_instance_extension_properties(66), capacity, len, buffer));
	...
	let mut xr = 0;
	ck(create_instance(&InstanceCreateInfo{application_info: ApplicationInfo{
		...
		extension_count: 2, extension_names: &[b"XR_KHR_D3D12_enable\0" as *const u8, b"XR_MSFT_holographic_remoting\0" as *const _] as *const _, ..default()}, &mut xr));
	...
	let mut system = 0;
	ck(get_system(xr, &SystemGetInfo{form_factor: FormFactor::HeadMounted, ..default()}, &mut system));
	...
	println!("xrGetD3D12GraphicsRequirementsKHR");
	...
	ck(get_D3D12_graphics_requirements(xr, system, &mut requirements)); // Microsoft Holographic Remoting implementation fails to create session without this call
	dbg!();
	...
	loop {
		...
		let (width, height) : (u32, u32) = (160, 120);
		...
		queue.write_texture(wgpu::ImageCopyTexture{texture: &gpu_image, mip_level: 0, origin: wgpu::Origin3d::ZERO, aspect: wgpu::TextureAspect::All},
					bytemuck::cast_slice(&image), wgpu::ImageDataLayout {offset: 0, bytes_per_row: Some(2 * size.width), rows_per_image: Some(size.height)},
					size);
